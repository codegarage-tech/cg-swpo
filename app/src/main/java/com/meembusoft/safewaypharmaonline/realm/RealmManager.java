package com.meembusoft.safewaypharmaonline.realm;
...
public class RealmManager {
    ...
    private static final String REALM_NAME = "swpo.realm";
...
//    }
    ...
    public <E extends RealmObject> List<E> getAllListData(Class<E> modelClass, Map<String, Object> keyValue) {
        try {
            RealmQuery<E> realmQuery = mRealm.where(modelClass);
            for (Map.Entry<String, Object> set : keyValue.entrySet()) {
                Object object = set.getValue();
                if (object instanceof String) {
                    realmQuery.equalTo(set.getKey(), (String) set.getValue());
                } else if (object instanceof Integer) {
                    realmQuery.equalTo(set.getKey(), (Integer) set.getValue());
                } else if (object instanceof Boolean) {
                    realmQuery.equalTo(set.getKey(), (Boolean) set.getValue());
                } else if (object instanceof Byte) {
                    realmQuery.equalTo(set.getKey(), (Byte) set.getValue());
                } else if (object instanceof byte[]) {
                    realmQuery.equalTo(set.getKey(), (byte[]) set.getValue());
                } else if (object instanceof Short) {
                    realmQuery.equalTo(set.getKey(), (Short) set.getValue());
                } else if (object instanceof Long) {
                    realmQuery.equalTo(set.getKey(), (Long) set.getValue());
                } else if (object instanceof Double) {
                    realmQuery.equalTo(set.getKey(), (Double) set.getValue());
                } else if (object instanceof Float) {
                    realmQuery.equalTo(set.getKey(), (Float) set.getValue());
                } else if (object instanceof Date) {
                    realmQuery.equalTo(set.getKey(), (Date) set.getValue());
                }
            }
            return mRealm.copyFromRealm(realmQuery.findAll());
        } catch (Exception e) {
            Log.d(TAG, "Failed to getAllData: " + e.getMessage());
        }
        return null;
    ...
    public <E extends RealmObject> void deleteData(Class<E> modelClass, Map<String, Object> keyValue) {
        try {
            ...
            if (!mRealm.isInTransaction())
                mRealm.beginTransaction();
            realmQuery.findFirst().deleteFromRealm();
            mRealm.commitTransaction();
        } catch (Exception e) {
            Log.d(TAG, "Failed to deleteData: " + e.getMessage());
        }
    ...
    public <E extends RealmObject> boolean isDataExist(Class<E> modelClass,  Map<String, Object> keyValue) {
        try {
            RealmQuery<E> realmQuery = mRealm.where(modelClass);
            for (Map.Entry<String, Object> set : keyValue.entrySet()) {
                Object object = set.getValue();
